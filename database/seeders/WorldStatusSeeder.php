...
class WorldStatusSeeder extends Seeder
{
    ...
    public function run()
    {
        $countriesArray = [ 'Algeria', 'Bahrain', 'Djibouti', 'Egypt', 'Iraq',
            'Jordan', 'Kuwait', 'Lebanon', 'Libya', 'Morocco',
            'Mauritania', 'Oman', 'Palestinian Territory Occupied', 'Qatar', 'Saudi Arabia',
            'Sudan', 'Syria', 'Tunisia', 'Yemen' ];
        Country::whereHas('states')->whereIn('name', $countriesArray)->update([ 'status' => true ]);
        State::select('states.*')->whereHas('cities')
