...
class PaymentController extends Controller
{
    ...
    public function cancelled($order_id)
    {
        ...
        toast('Your order has been cancelled', 'error');
        return redirect()->route('frontend.index');
    }
    ...
    public function completed($order_id)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            $order_data = Order::with('products', 'user', 'payment_method')->find($order_id);
            $data = $order_data->toArray();
            $data[ 'currency_symbol' ] = $order_data->currency == 'USD' ? '$' : $order_data->currency;
            $pdf = PDF::loadView('layouts.invoice', $data);
            $saved_file = storage_path('app/pdf/files/' . $data[ 'ref_id' ] . '.pdf');
            $pdf->save($saved_file);
            $customer = User::find($order->user_id);
            $customer->notify(new OrderThanksNotification($order_data, $saved_file));
            toast('Your recent payment is successful with response code: ' . $response->getTransactionReference(), 'success');
            return redirect()->route('frontend.index');
        }
    }
